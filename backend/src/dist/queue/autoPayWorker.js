...
const autoPayQueue_1 = require("./autoPayQueue");
...
function initializeWorker() {
    try {
        const queue = (0, autoPayQueue_1.autoPayQueue)();
        queue.process(async (job) => {
            ...
            try {
                ...
                const hasTooManyFailures = await failedPaymentTracker_1.failedPaymentTracker.hasTooManyFailures(subscriptionId, 3);
                ...
                const paymentResult = await processPayment(subscriptionId, userAddress, amount, recipientAddress, attemptNumber);
                // If payment was successful, record it
                if (paymentResult.success && paymentResult.transactionHash) {
                    await job.progress(80);
                    await subscriptionService.recordPayment(subscriptionId, amount, paymentResult.transactionHash, 'hedera-testnet', 'completed');
                    ...
                    await subscriptionService.recordPayment(subscriptionId, amount, '', 'hedera-testnet', 'failed', `${categorized.category}: ${categorized.message}`);
                    // Track the failure
                    const nextRetryAt = (0, paymentErrors_1.shouldRetry)(categorized, attemptNumber)
                        ? new Date(Date.now() + (0, paymentErrors_1.calculateRetryDelay)(categorized, attemptNumber))
                        : undefined;
                    await failedPaymentTracker_1.failedPaymentTracker.recordFailure(subscriptionId, userAddress, amount, categorized.originalError, attemptNumber, nextRetryAt);
                    // If retryable and within max attempts, let Bull handle the retry
                    if ((0, paymentErrors_1.shouldRetry)(categorized, attemptNumber)) {
                        console.log(`[AUTO_PAY_WORKER] ⚠️ Payment failed (retryable): ${categorized.message}`);
                        console.log(`[AUTO_PAY_WORKER] Will retry in ${(0, paymentErrors_1.calculateRetryDelay)(categorized, attemptNumber)}ms`);
                        ...
                        throw new Error((0, paymentErrors_1.getUserFriendlyMessage)(categorized));
                    }
                }
            }
            catch (error) {
                const errorObj = error instanceof Error ? error : new Error(String(error));
                const categorized = (0, paymentErrors_1.categorizePaymentError)(errorObj);
 ...
 */
async function processPayment(subscriptionId, userAddress, amount, recipientAddress, attemptNumber) {
    ...
    if (attemptNumber === 1) {
        ...
        return {
            ...
            errorCategory: paymentErrors_1.ErrorCategory.NETWORK_ERROR,
        };
    }
    ...
    return {
        ...
        errorCategory: paymentErrors_1.ErrorCategory.WALLET_ERROR,
    };
}
